...
const Individual = ({
    ...
    return (
        <a href={barberUrl} target="_blank" class="m-2 p-4 sm:m-3 sm:p-3 md:m-4 hover:bg-gray-100 rounded-lg shadow">
            <div class="text-center mb-4 opacity-90 block relative">
                <img alt={name} src="http://placecorgi.com/472/343" class="mx-auto object-cover rounded-full h-40 w-40 " />
            </div>
            <div class="text-center">
                <p class="text-2xl text-gray-800">
                    {name}
                </p>
                <p class="max-w-xs my-4">
                    {description}
                </p>
                <button type="button" class="border-gray-800 hover:border-gray-900 border-2 focus:ring-4 focus:ring-gray-300 rounded-lg text-sm px-5 py-2.5 text-center">{`Book with ${name.split(' ')[0]}`}</button>
            </div>
            {(facebook || instagram) && <div class="pt-4 mt-4 flex border-t border-gray-200 w-44 mx-auto text-gray-500 items-center justify-around">
                {facebook && <a href={facebook} target="_blank">
                    <svg width="30" height="30" fill="currentColor" class="text-xl hover:text-gray-800 transition-colors duration-200" viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg">
                        <path d="M1343 12v264h-157q-86 0-116 36t-30 108v189h293l-39 296h-254v759h-306v-759h-255v-296h255v-218q0-186 104-288.5t277-102.5q147 0 228 12z">
                        </path>
                    </svg>
                </a>}
                {instagram && <a href={instagram} target="_blank">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="text-xl hover:text-gray-800 transition-colors duration-200" width="27" height="27" viewBox="0 0 512 512" style={{ "enable-background": "new 0 0 512 512" }} xmlspace="preserve">
